...
# order factor levels
measures_teams_nhl$measure <- factor(measures_teams_nhl$measure,
  levels = c("Idade", "Peso", "Altura")
)
...
p <- measures_teams_nhl %>%
  ...
  geom_text(aes(label = paste0("(", round(mean_measure, 2), ")")),
    size = 1.75, # Adjust text size
    vjust = -0.5, # Adjust vertical position
    hjust = 0.5,
    nudge_x = 0.04, # Adjust horizontal position
    color = "gray10",
  ...
  # tweak x-axis
  xlim(-0.18, 0.18) + # scale_x_continuous(breaks = seq(-0.15, 0.05, 0.15)) +
  ...
  labs(
    ...
    y = "",
    title = "Altura, Peso e Idade Media dos Jogadores por Time",
    subtitle = paste0(
      "Comparando as medias dos times em relacao a altura (",
      round(height_df$league_average[1], 2), " cm) , peso (",
      round(weight_df$league_average[1], 2), " kg) e idade (",
      round(age_df$league_average[1], 2), ") media da liga"
    )
  )


ggsave("/Users/danilooak/Documents/Code/R Projects/Sports Analytics/Hockey/imgs/roster_measures.png", # nolint
  p,
  width = 10, height = 8.0, dpi = 300
)
