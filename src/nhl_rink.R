...
gg_rink <- function(side = "right", specs = "nhl"){
  ...
  switch(specs,
         "nhl" = {
           ...
           ### rink boundaries ###
           ## assumed to be standard 300x85ft dimensions
           x_max <- 100
           y_max <- 42.5
           y_min <- -y_max
  ...
  list(
    ...
    switch(specs,
           "nhl" = annotate(
             ...
             x = 0, y = y_max,
             xend = 0, yend = y_min,
             color = "white", size = 0.5/3, linetype = "dashed"
           ),
           "iihf" = annotate(
